                    ...
                    <form class="space-y-5" @submit.prevent="processAction(page.primary?.to)">
                        <div v-if="slug !== 'otp-verification'" class="space-y-1">
                            <label for="account-email"
                                ...
                                Address</label>
                            <input id="account-email" type="email" placeholder="john.doe@example.com"
                        ...
                        <div v-if="slug === 'otp-verification'" class="flex justify-center gap-3">
                            <input v-for="index in 4" :key="index" maxlength="1" inputmode="numeric"
        ...
        <main class="flex-grow">
            <section v-if="page.type === 'showroom'" class="relative overflow-hidden bg-tccDeepBlack text-white">
                <div class="absolute inset-0">
                    <img src="/generated/hero-aston-studio.png" alt="Classic car showroom"
                        class="h-full w-full object-cover opacity-60">
                    <div
                        class="absolute inset-0 bg-gradient-to-b from-tccDeepBlack/60 via-tccDeepBlack/70 to-tccDeepBlack" />
                </div>
                <div
                    class="relative mx-auto flex min-h-[520px] max-w-7xl items-center px-4 py-16 sm:min-h-[560px] sm:px-6 sm:py-20 lg:px-8">
            ...
            <section v-if="page.type === 'showroom' || page.type === 'asset-list'" id="live-grid"
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
                    <article v-for="asset in assetCards" :key="asset.title"
                        class="restomod-image-card overflow-hidden rounded-[1.5rem] border border-tccBorder bg-white shadow-sm">
                ...
                <div class="restomod-panel overflow-hidden rounded-[2rem]">
                    <div class="relative h-[380px] bg-tccDeepBlack sm:h-[420px]">
                        ...
                        <div class="absolute bottom-0 left-0 min-w-0 p-4 text-white sm:p-10">
                            <span class="restomod-eyebrow">{{ page.eyebrow }}</span>
                            <h1 class="mt-4 font-poppins text-4xl font-black leading-tight sm:text-5xl">{{ page.title }}
                            </h1>
                    ...
                    <div class="grid grid-cols-1 gap-8 p-6 lg:p-10">
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <img src="/frontend/assets/images/aston_martin_vanquish.png" alt="Gallery asset"
                                    class="h-40 w-full rounded-lg object-cover">
                                <img src="/frontend/assets/images/ferrari_berlinetta.png" alt="Gallery asset"
                                    class="h-40 w-full rounded-lg object-cover">
                                <img src="/frontend/assets/images/porsche.png" alt="Gallery asset"
                                    class="h-40 w-full rounded-lg object-cover">
                                <img src="/generated/restoration-workshop.png" alt="Workshop"
                                    class="h-40 w-full rounded-lg object-cover">
                            </div>
                        </div>
                    </div>
            ...
            <section v-else class="mx-auto flex max-w-5xl items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
                <div class="restomod-panel w-full rounded-[2rem] p-6 sm:p-10">
                    ...
                    <div v-if="page.type === 'status'" class="mx-auto mt-8 max-w-2xl text-center">
                        <div class="mx-auto flex h-20 w-20 items-center justify-center rounded-full border text-3xl"
                    ...
                    <form v-else-if="page.type === 'form' || page.type === 'contact'"
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="space-y-1">
                                <label class="block text-xs font-semibold uppercase tracking-wider text-tccNavy">Last
                                    Name</label>
                                <input class="w-full rounded-lg border border-tccBorder px-4 py-2.5 text-sm">
                            </div>
                        </div>
                        <div class="space-y-1">
                            <label class="block text-xs font-semibold uppercase tracking-wider text-tccNavy">Email
                                Address</label>
                        ...
                        <button type="submit"
                            class="w-full rounded-lg bg-tccGold px-6 py-3.5 font-poppins text-xs font-bold uppercase tracking-wider text-tccDarkNavy">
                            {{ page.primary?.label || 'Continue' }}
                        </button>
                    </form>

                    <div v-else-if="page.type === 'checkout'"
                        class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
                        <div class="space-y-5 rounded-lg border border-tccBorder bg-stone-50 p-5">
                            <div class="flex flex-col gap-4 sm:flex-row">
                                <img src="/frontend/assets/images/ferrari_berlinetta.png" alt="Selected allocation"
                                    class="h-32 w-full rounded-lg object-cover sm:w-44">
                                <div>
                                    <h2 class="font-poppins text-lg font-semibold text-tccNavy">Ferrari 365 GTB/4
                                        Daytona</h2>
                                    <p class="mt-2 text-xs font-light leading-relaxed text-gray-600">One reserved
                                        syndicate slot with trust beneficiary rights and member voting access.</p>
                                </div>
                            </div>

                            <div v-if="slug === 'payment' || slug === 'payment-details'"
                                class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <button type="button" class="rounded-lg border p-4 text-left"
                                    :class="selectedPayment === 'bank' ? 'border-tccNavy bg-white' : 'border-tccBorder'"
                                ...
                                <button type="button" class="rounded-lg border p-4 text-left"
                                    :class="selectedPayment === 'crypto' ? 'border-tccNavy bg-white' : 'border-tccBorder'"
                            ...
                            <div
                                class="rounded-lg border border-tccBorder bg-white p-4 text-xs leading-relaxed text-gray-600">
                        ...
                        <aside class="h-fit space-y-4 rounded-lg border border-tccBorder bg-white p-5">
                            ...
                            <div class="space-y-2 text-xs text-gray-600">
                                <div class="flex justify-between"><span>Allocation</span><strong>GBP 8,500.00</strong>
                                </div>
                    ...
                    <div v-else-if="page.type === 'vote'" class="mx-auto mt-8 max-w-2xl space-y-5">
                        <label
                            v-for="option in ['Approve proposed acquisition terms', 'Request further information', 'Abstain from this vote']"
                            :key="option" class="flex cursor-pointer items-start gap-3 rounded-lg border p-4"
                            :class="selectedVote === option ? 'border-tccNavy bg-tccLightBg' : 'border-tccBorder'">
                            <input v-model="selectedVote" type="radio" :value="option" class="mt-1 accent-tccNavy">
                            <span class="text-sm text-gray-700">{{ option }}</span>
                        </label>
                        <button type="button"
                            class="w-full rounded bg-tccGold px-5 py-3 font-poppins text-xs font-bold uppercase tracking-wider text-tccDarkNavy"
                            @click="processAction(page.primary?.to)">
                            {{ page.primary?.label }}
                        </button>
                    </div>
                    ...
                    <div v-else class="mx-auto mt-8 max-w-3xl space-y-5">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div v-for="item in ['Subscription Deed', 'Trust Summary', 'Risk Disclosure']" :key="item"
                                ...
                                <p class="mt-2 text-xs font-light leading-relaxed text-gray-600">Mapped from the
                                    supplied document flow and connected to the next portal step.</p>
                            </div>
                        </div>
