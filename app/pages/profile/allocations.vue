      ...
      <section class="space-y-5 rounded-2xl border border-white/12 bg-white/[0.06] p-5 text-white shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p class="mt-1 text-[13px] text-white/55">Track active holdings, pending payments, or closed syndicates.</p>
          </div>
          <div class="rounded-xl border border-tccGold/25 bg-tccGold/10 px-4 py-2.5 text-[13px] font-semibold text-white">
              ...
              <tr v-for="allocation in allocations" :key="allocation.code" class="transition-colors hover:bg-white/[0.035]">
                <td class="p-3.5">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h3 class="font-poppins text-[13px] font-black text-white">{{ allocation.title }}</h3>
                ...
                <td class="p-3.5 text-[10px] font-black uppercase tracking-[0.1em]" :class="verificationClass(allocation.verificationTone)">
                  <i class="pi pi-check-circle mr-1" aria-hidden="true" />
                  {{ allocation.verification }}
                </td>
                <td class="p-3.5 text-right">
                  <NuxtLink :to="allocation.actionRoute" class="inline-flex items-center gap-2 rounded-full border border-white/15 px-3.5 py-2 font-poppins text-[10px] font-black uppercase tracking-[0.12em] text-white transition-colors hover:border-tccGold hover:text-tccGold">
                    {{ allocation.action }}
