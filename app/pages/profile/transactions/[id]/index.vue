...
<template>
  <section class="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <NuxtLink to="/profile/transactions" class="inline-flex items-center gap-2 font-poppins text-xs font-black uppercase tracking-[0.16em] text-white/55 transition-colors hover:text-tccGold">
      ...
      <div class="flex flex-col gap-3 sm:flex-row">
        <NuxtLink
          ...
          class="inline-flex items-center justify-center gap-2 rounded-full border border-tccGold/35 bg-tccGold/10 px-5 py-3 font-poppins text-[11px] font-black uppercase tracking-[0.14em] text-tccGold transition-colors hover:bg-tccGold hover:text-tccDarkNavy"
        >
        ...
        <NuxtLink
          ...
          class="inline-flex items-center justify-center gap-2 rounded-full bg-tccGold px-5 py-3 font-poppins text-[11px] font-black uppercase tracking-[0.14em] text-tccDarkNavy shadow-lg shadow-tccGold/20 transition-colors hover:bg-tccLightGold"
        >
    ...
    <section class="overflow-hidden rounded-2xl border border-white/12 bg-white/[0.06] p-5 text-white shadow-[0_28px_90px_rgba(0,0,0,0.34)] sm:p-7">
      <div class="grid gap-5 lg:grid-cols-2">
        <article class="rounded-2xl border border-white/10 bg-tccDeepBlack/60 p-5">
          <div class="flex items-center gap-3">
            ...
            <h1 class="font-poppins text-xl font-black text-white">Vehicle Information</h1>
          </div>

          <div class="mt-5 overflow-hidden rounded-xl border border-white/10 bg-[#0b0a08]">
            <img :src="transaction.image" :alt="transaction.carName" class="h-72 w-full object-cover opacity-90">
          </div>

          <div class="mt-5 space-y-4">
            <div>
              <span class="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Car Name</span>
              <h2 class="mt-1 font-poppins text-lg font-black text-white">{{ transaction.carName }}</h2>
            </div>
            <div>
              <span class="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Description</span>
              <p class="mt-2 text-sm leading-relaxed text-white/72">{{ transaction.description }}</p>
            </div>
          </div>
        </article>
        ...
        <article class="rounded-2xl border border-tccGold/20 bg-[radial-gradient(circle_at_top_right,rgba(247,198,0,0.12),transparent_20rem),#0b0a08] p-5">
          <div class="flex items-center gap-3">
            ...
            <h2 class="font-poppins text-xl font-black text-white">Purchase Details</h2>
          </div>
          ...
          <div class="mt-6 grid gap-4 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/[0.04] p-4">
              ...
              <strong class="mt-2 inline-flex rounded-lg bg-tccGold/10 px-3 py-2 text-sm text-tccGold">
                {{ transaction.allocations }} slot
              </strong>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/[0.04] p-4">
              <span class="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Reference</span>
              <strong class="mt-2 block font-mono text-sm text-tccGold">{{ transaction.referenceId }}</strong>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/[0.04] p-4">
              <span class="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Total Investment</span>
              <strong class="mt-2 block font-poppins text-2xl font-black text-tccGold">{{ formatCurrency(transaction.totalInvestment) }}</strong>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/[0.04] p-4">
            ...
            <div class="rounded-xl border border-white/10 bg-white/[0.04] p-4">
              ...
              <strong class="mt-2 block text-sm text-white">{{ transaction.signedDate }}</strong>
            </div>
          </div>

          <div class="mt-6 rounded-xl border border-tccGold/25 bg-tccGold/10 p-4 text-sm leading-relaxed text-white/70">
