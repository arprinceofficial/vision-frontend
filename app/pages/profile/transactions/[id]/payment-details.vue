...
<template>
  <section class="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="mb-8 text-center">
      ...
      <h1 class="mt-4 font-poppins text-3xl font-black text-white sm:text-4xl">Bank Transfer Details</h1>
      <p class="mt-2 text-sm text-white/55">Please transfer the exact amount to the account below</p>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <div class="space-y-5">
        <section class="rounded-2xl border border-tccGold/20 bg-[#11100d] p-5 text-white shadow-[0_22px_80px_rgba(0,0,0,0.26)]">
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-[11rem_minmax(0,1fr)]">
            ...
            <div class="space-y-3">
              <div class="flex flex-col gap-1 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
              ...
              <div class="flex items-center justify-between rounded-xl border border-tccGold/35 bg-tccGold px-4 py-4 text-tccDarkNavy">
                <span class="font-poppins text-sm font-black">Total Amount</span>
                <strong class="font-poppins text-2xl font-black">{{ formatCurrency(transaction.totalInvestment) }}</strong>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="rounded-2xl border border-white/10 bg-white/[0.05] p-5 text-white">
          <h2 class="font-poppins text-xl font-black">Important Information</h2>
      ...
      <section class="rounded-2xl border border-white/10 bg-white/[0.05] p-5 text-white shadow-[0_22px_80px_rgba(0,0,0,0.26)] lg:self-start">
        <h2 class="font-poppins text-xl font-black">Bank Account Details</h2>
        ...
        <div class="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          >
            ...
            <div class="flex items-center gap-3 rounded-xl border border-white/10 bg-tccDeepBlack/70 px-4 py-3">
        ...
        <div class="mt-5 rounded-2xl border border-tccGold/50 bg-tccGold/10 p-4">
          <div class="mb-2 flex flex-wrap items-center gap-2">
            <span class="text-[11px] font-black uppercase tracking-[0.14em] text-white"># Payment Reference</span>
            <span class="text-[11px] font-black text-tccGold">(Required)</span>
          </div>
          <div class="flex items-center gap-3 rounded-xl border border-tccGold/40 bg-tccDeepBlack px-4 py-3">
            <strong class="min-w-0 flex-1 break-words font-mono text-lg text-tccGold">{{ transaction.referenceId }}</strong>
            <button
              type="button"
              class="inline-flex shrink-0 items-center gap-2 rounded-lg bg-tccGold px-4 py-2 font-poppins text-xs font-black uppercase tracking-[0.12em] text-tccDarkNavy transition-colors hover:bg-tccLightGold"
        ...
        <div class="mt-5">
          <span class="text-[11px] font-black uppercase tracking-[0.14em] text-white/45">Amount to Transfer</span>
          <div class="mt-2 flex items-center gap-3 rounded-xl border border-white/10 bg-tccDeepBlack/70 px-4 py-3">
            <strong class="min-w-0 flex-1 break-words font-mono text-lg text-tccGold">{{ formatCurrency(transaction.totalInvestment) }}</strong>
            <button
              type="button"
              class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-tccGold text-tccDarkNavy transition-colors hover:bg-tccLightGold"
