                ...
                <div
                    ...
                    <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                        <div>
                            ...
                            <p class="mt-2 max-w-2xl text-[13px] leading-relaxed text-white/60">
                                Review pending transfers, funded allocations, references, and the next action for each
                                syndicate purchase.
                            </p>
                        </div>
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-3">
                        <div class="rounded-xl border border-white/10 bg-white/[0.05] p-3.5">
                            <span
                                class="text-[10px] font-black uppercase tracking-[0.18em] text-white/45">Allocations</span>
                            <strong class="mt-1.5 block font-poppins text-xl font-black text-white">{{
                                transactionSummary.total }}</strong>
                        </div>
                        <div class="rounded-xl border border-orange-400/25 bg-orange-400/10 p-3.5">
                            <span
                                class="text-[10px] font-black uppercase tracking-[0.18em] text-orange-100/60">Pending</span>
                            <strong class="mt-1.5 block font-poppins text-xl font-black text-orange-100">{{
                                transactionSummary.pending }}</strong>
                        </div>
                        <div class="rounded-xl border border-tccGold/30 bg-tccGold/10 p-3.5">
                            <span
                                class="text-[10px] font-black uppercase tracking-[0.18em] text-tccGold/80">Funded</span>
                            <strong class="mt-1.5 block font-poppins text-xl font-black text-tccGold">{{
                                transactionSummary.funded }}</strong>
                        </div>
                    </div>
                </div>
                                ...
                                <tr v-for="transaction in transactions" :key="transaction.id"
                                    ...
                                    <td class="p-3.5">
                                        <div class="flex items-center gap-4">
                                            <span
                                                class="grid h-16 w-24 shrink-0 place-items-center overflow-hidden rounded-lg border border-white/10 bg-tccDeepBlack">
                                                <img :src="transaction.image" :alt="transaction.vehicle"
                                                    class="h-full w-full object-cover opacity-85">
                                            </span>
                                            <span class="min-w-0">
                                                <strong
                                                    class="line-clamp-1 block font-poppins text-[13px] font-black text-white">
                                                    {{ transaction.allocationDetails }}
                                                </strong>
                                                <span class="mt-1 block text-[11px] text-white/50">{{
                                                    transaction.collection }}</span>
                                            </span>
                                        </div>
                                    </td>
                                    <td class="p-3.5">
                                    ...
                                    <td class="p-3.5">
                                        <span
                                            class="inline-flex items-center gap-2 rounded-full border px-2.5 py-1.5 text-[10px] font-black"
                                            :class="transactionStatusClass(transaction.allocationTone)">
                                    ...
                                    <td class="p-3.5">
                                        <span
                                            class="inline-flex items-center gap-2 rounded-full border px-2.5 py-1.5 text-[10px] font-black"
                                            :class="paymentStatusClass(transaction.paymentTone)">
                                            <i class="pi"
