...
<template>
    <div class="bg-tccDeepBlack text-white">
        <template v-if="shouldShowRetailCarSkeleton">
            ...
            <section
                class="relative left-[50%] right-[50%] ml-[-50vw] mr-[-50vw] w-screen max-w-none overflow-hidden border-b border-white/10 bg-tccDeepBlack text-white">
                <div class="absolute left-1/2 top-0 h-full w-screen -translate-x-1/2" aria-hidden="true">
                    ...
                    <div
                        class="absolute inset-0 bg-gradient-to-b from-tccDeepBlack/30 via-tccDeepBlack/70 to-tccDeepBlack" />
                    <div
                        class="absolute inset-0 bg-[radial-gradient(circle_at_66%_34%,transparent_0%,rgba(3,3,3,0.38)_34%,rgba(3,3,3,0.96)_100%)]" />
                </div>
                ...
                <div class="relative z-10 mx-auto max-w-7xl px-4 pb-16 pt-24 sm:px-6 lg:px-8 lg:pb-20 lg:pt-28">
                    ...
                    <div class="mt-10 max-w-5xl">
                        ...
                        <div class="mt-6 space-y-4">
                            <span
                                class="block h-12 w-full max-w-4xl animate-pulse rounded-xl bg-white/10 sm:h-16 lg:h-20" />
                            <span
                                class="block h-12 w-4/5 max-w-3xl animate-pulse rounded-xl bg-white/10 sm:h-16 lg:h-20" />
                        </div>
                        ...
                        <div class="mt-9 grid max-w-4xl grid-cols-2 gap-4 md:grid-cols-4">
                            <div v-for="statWidth in detailSkeletonStats" :key="statWidth"
                                class="border-l border-tccGold/50 pl-4">
                                <span class="block h-3 w-16 animate-pulse rounded-full bg-white/10" />
                                <strong class="mt-2 block h-4 animate-pulse rounded-full bg-white/10"
                                    :class="statWidth" />
                            </div>
                        </div>
                    </div>
            ...
            <section class="border-b border-white/10 bg-[#080705] py-16 text-white">
                <div
                    class="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 sm:px-6 lg:grid-cols-[minmax(0,0.58fr)_minmax(360px,0.42fr)] lg:px-8">
                    ...
                    <aside class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                        <div class="restomod-panel rounded-[1.75rem] p-6">
                            ...
                            <div class="mt-6 overflow-hidden rounded-[1rem] border border-white/10">
                                <div v-for="(specWidth, index) in detailSkeletonSpecs" :key="`${specWidth}-${index}`"
                                    ...
                                    <strong class="px-4 py-3">
                                        <span class="block h-3 animate-pulse rounded-full bg-white/10"
                                            :class="specWidth" />
                                    </strong>
                                </div>
                            </div>
                        ...
                        <div class="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
                            <span class="block h-4 w-32 animate-pulse rounded-full bg-tccGold/25" />
                            <div class="mt-6 space-y-4">
                                <div v-for="highlightWidth in detailSkeletonHighlights" :key="highlightWidth"
            ...
            <section class="bg-tccDeepBlack py-16 text-white">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div v-for="image in 3" :key="image"
                            class="restomod-image-card overflow-hidden rounded-[1.25rem] border border-white/10 bg-white/5">
        ...
        <template v-else-if="car">
            <section
                class="relative left-[50%] right-[50%] ml-[-50vw] mr-[-50vw] w-screen max-w-none overflow-hidden border-b border-white/10 bg-tccDeepBlack text-white">
                <div class="absolute left-1/2 top-0 h-full w-screen -translate-x-1/2" aria-hidden="true">
                    <div class="h-full w-full bg-cover bg-center bg-no-repeat opacity-55"
            ...
            <section class="border-b border-white/10 bg-[#080705] py-16 text-white">
                <div
                    ...
                    <article class="min-w-0">
                        <div
                            class="restomod-image-card overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/5">
                            <img :src="car.gallery.exterior[0] || car.cardImage" :alt="car.title"
                                class="h-[320px] w-full object-cover sm:h-[500px]">
                        </div>
                        ...
                        <div class="mt-10 space-y-8">
                            <section v-for="section in car.sections" :key="section.heading">
                                ...
                                <div v-if="section.paragraphs"
                                    class="mt-4 space-y-4 text-base leading-relaxed text-white/70">
                                    <p v-for="paragraph in section.paragraphs" :key="paragraph">{{ paragraph }}</p>
                                </div>
                                <ul v-if="section.bullets"
                                    class="mt-4 space-y-2 text-sm leading-relaxed text-white/70 sm:text-base">
                    ...
                    <aside class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                        <div class="restomod-panel rounded-[1.75rem] p-6">
                            <span
                                class="font-poppins text-[10px] font-bold uppercase tracking-[0.24em] text-tccGold">Specification</span>
                            <div class="mt-6 overflow-hidden rounded-[1rem] border border-white/10">
                        ...
                        <div v-if="car.highlights.length"
                            class="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
                            <span
                                class="font-poppins text-[10px] font-bold uppercase tracking-[0.24em] text-tccGold">Key
                                Highlights</span>
                            <div class="mt-6 space-y-4">
                                <div v-for="highlight in car.highlights" :key="highlight"
                                    class="flex gap-3 border-t border-white/10 pt-4 first:border-t-0 first:pt-0">
            ...
            <section class="bg-tccDeepBlack py-16 text-white">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid auto-rows-[220px] grid-cols-1 gap-4 md:grid-cols-4">
                        <div v-for="(image, index) in car.gallery.exterior" :key="`${image}-${index}`"
                            class="restomod-image-card overflow-hidden rounded-[1.25rem] border border-white/10 bg-white/5"
                            :class="index === 0 ? 'md:col-span-2 md:row-span-2' : index === 2 ? 'md:row-span-2' : ''">
                            <img :src="image" :alt="`${car.title} exterior ${index + 1}`"
                    ...
                    <template v-if="car.gallery.interior.length">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div v-for="(image, index) in car.gallery.interior" :key="`${image}-${index}`"
                                class="restomod-image-card overflow-hidden rounded-[1.25rem] border border-white/10 bg-white/5">
                                <img :src="image" :alt="`${car.title} interior ${index + 1}`"
            ...
            <section v-if="shouldShowRelatedCarsSkeleton || relatedCars.length"
                class="border-t border-white/10 bg-[#080705] py-16 text-white">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                    ...
                    <div v-else class="grid grid-cols-1 gap-6 md:grid-cols-3">
                        <NuxtLink v-for="relatedCar in relatedCars" :key="relatedCar.slug"
                            ...
                            <div class="relative h-52 overflow-hidden bg-tccDeepBlack">
                                <img :src="relatedCar.cardImage" :alt="relatedCar.title"
