...
watch(() => props.item, (value) => {
    if (value && Object.keys(value).length > 0) {
        ...
        formData.value = {
            ...
            year_from: toInputValue(value.year_from),
...
const setGallery = (photos) => {
    formData.value.gallery = normalizeGallery(photos);
};
...
const serializeSubmitData = () => ({
    ...
    condition_grade: formData.value.condition_grade,
    gallery: normalizeGallery(formData.value.gallery),
    ...
    year_from: formData.value.year_from,
    three_d_image_url: formData.value.three_d_image_url,
    video_path: formData.value.video_path,
});
...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/vehicle-details/${props.item.id}`, {
            method: 'PUT',
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin('v1/admin/vehicle-details', {
            method: 'POST',
            body: serializeSubmitData(),
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Vehicle Detail</h4>
            </div>
        </template>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <LazyInputText v-model="formData.mileage" type="number" class="w-full"
                    :class="validations_errors.mileage ? 'border-[#f44336!important]' : ''" autocomplete="off"
            ...
            <div>
                ...
                <LazyInputText v-model="formData.year_from" type="number" class="w-full"
                    :class="validations_errors.year_from ? 'border-[#f44336!important]' : ''" autocomplete="off"
