...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/icons/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`v1/admin/icons/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/icons/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
    ...
    <div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
        <div class="h-full w-full overflow-auto">
            <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
                ...
                <Button v-else-if="permissions?.add" label="Create Icon" @click="addNew" class="text-xs" />
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Icons</h4>
                    </div>
                    <div class="p-4">
                            ...
                            <table class="table table-auto">
                                ...
                                <tbody v-if="isLoading">
                                    <tr v-for="(index) in 10" :key="index">
                                        <td><div class="flex justify-center"><Skeleton size="3rem"></Skeleton></div></td>
                                    ...
                                    <tr v-for="(item, index) in data" :key="index">
                                        <td>
                                            <div class="flex justify-center">
                                                <img :src="item.bg_image" alt="icon background" class="w-10 h-10 object-cover rounded"
                                                    v-if="item.bg_image" />
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200">{{ item.category?.title || '-' }}</td>
                                        <td>
                                            <div class="flex justify-center items-center">
                                                <span v-if="item.is_home == 1" class="text-green-600"><i class="fa fa-check" aria-hidden="true"></i></span>
                                                <span v-else class="text-gray-400"><i class="fa fa-minus" aria-hidden="true"></i></span>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="flex justify-center items-center">
