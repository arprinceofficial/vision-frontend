...
const planData = async () => {
    try {
        const getData = await $fetchAdmin(`v1/admin/plans/all`, {
            method: 'POST',
        });
...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/customer-reviews/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`v1/admin/customer-reviews`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
