...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/social-links/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`v1/admin/social-links/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/social-links/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
