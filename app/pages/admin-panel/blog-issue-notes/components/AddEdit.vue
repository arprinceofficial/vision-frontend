...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/blog-issue-notes/${props.item.id}`, {
            method: 'PUT',
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin('v1/admin/blog-issue-notes', {
            method: 'POST',
            body: serializeSubmitData(),
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '44rem', maxWidth: 'calc(100vw - 2rem)' }">
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Blog Issue Note</h4>
            </div>
        </template>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <LazyInputText v-model="formData.label" class="w-full"
                    :class="validations_errors.label ? 'border-[#f44336!important]' : ''" autocomplete="off"
                    @focus="validations_errors.label = ''" />
                <LazyInputError class="text-sm mt-1" :message="validations_errors.label" />
            </div>

            <div>
                <label class="font-semibold">Value</label>
                <LazyInputText v-model="formData.value" class="w-full"
                    :class="validations_errors.value ? 'border-[#f44336!important]' : ''" autocomplete="off"
                    @focus="validations_errors.value = ''" />
                <LazyInputError class="text-sm mt-1" :message="validations_errors.value" />
            </div>

            <div class="sm:col-span-2">
                <label class="font-semibold">Detail</label>
                <Textarea v-model="formData.detail" class="w-full" rows="5"
                    :class="validations_errors.detail ? 'border-[#f44336!important]' : ''" autocomplete="off"
                    @focus="validations_errors.detail = ''" />
                <LazyInputError class="text-sm mt-1" :message="validations_errors.detail" />
            </div>
