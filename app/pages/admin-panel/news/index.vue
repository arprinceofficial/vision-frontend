...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/news/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const loadFaqCategories = async () => {
    isLoading.value = true;
    try {
        const getData = await $fetchAdmin(`v1/admin/news-categories`, {
            method: 'GET',
        });
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`v1/admin/news/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/news-categories/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
