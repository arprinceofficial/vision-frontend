...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/tags/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`v1/admin/tags/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/tags/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
