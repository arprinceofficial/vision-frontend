...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/investment-cards/${props.item.id}`, {
            method: 'PUT',
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin('v1/admin/investment-cards', {
            method: 'POST',
            body: serializeSubmitData(),
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '44rem', maxWidth: 'calc(100vw - 2rem)' }">
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Investment Card</h4>
            </div>
        </template>
