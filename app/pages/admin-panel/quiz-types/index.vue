...
const loadData = async () => {
    ...
    try {
        ...
        let getData;
        try {
            getData = await $fetchAdmin('v1/admin/onboarding/quiz-types/all', {
                method: 'POST',
                body: requestBody,
            });
        } catch (postError) {
            ...
            getData = await $fetchAdmin('v1/admin/onboarding/quiz-types', {
                method: 'GET',
                query: requestBody,
            });
        }
        ...
        response_modal.value = {
            status: false,
            message: e?.response?._data?.message || 'Failed to load onboarding quiz types.',
        };
    } finally {
...
const deleteHandler = async () => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/onboarding/quiz-types/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/onboarding/quiz-types/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
    ...
    <div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
        <div class="h-full w-full overflow-auto">
            <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
                ...
                <Button v-else-if="permissions?.add" label="Create Quiz Type" @click="addNew" class="text-xs" />
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Onboarding Quiz Types</h4>
                    </div>
                            ...
                            <table class="table table-auto">
                                ...
                                <tbody v-else>
                                    <tr v-for="(row, index) in data" :key="index">
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200 text-start">
                                            <span class="text-sm">{{ row.slug }}</span>
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-start">
                                            <div class="max-w-md truncate" :title="row.description">
                                                {{ row.description || 'N/A' }}
                                            </div>
                                        </td>
                                        <td>
                                    ...
                                    <tr v-if="data.length === 0">
                                        <td :colspan="permissions.edit || permissions.delete ? 5 : 4" class="text-center text-sm text-gray-500 py-6">
                                            No onboarding quiz types found.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
