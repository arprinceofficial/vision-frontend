...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/syndicate-how-it-works/${props.item.id}`, {
            method: 'PUT',
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin('v1/admin/syndicate-how-it-works', {
            method: 'POST',
            body: serializeSubmitData(),
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '44rem', maxWidth: 'calc(100vw - 2rem)' }">
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Syndicate How It Works</h4>
            </div>
        </template>
