...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin('v1/admin/retail-cars/all', {
            method: 'POST',
            body: {
                paginate: true,
...
const displayMakeModel = (item) => [item.make, item.model].filter(Boolean).join(' ');
const displayStatus = (item) => item.retail_status || item.status || '';
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`v1/admin/retail-cars/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`v1/admin/retail-cars/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
    ...
    <div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
        <div class="h-full w-full overflow-auto">
            <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
                ...
                <Button v-else-if="permissions?.add" label="Create Retail Car" @click="addNew" class="text-xs" />
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Retail Cars</h4>
                    </div>
                    <div class="p-4">
                            ...
                            <table class="table table-auto">
                                <thead class="sticky z-10 top-0">
                                    <tr>
                                        ...
                                        <th width="14%"><span>Retail Status</span></th>
                                        <th width="14%"><span>Price</span></th>
                                        <th width="8%"><span>Year</span></th>
                                        <th width="16%"><span>Make / Model</span></th>
                                        <th width="12%"><span>Mileage</span></th>
                                        <th width="12%"><span>Location</span></th>
                                ...
                                <tbody v-if="isLoading">
                                    <tr v-for="(index) in 10" :key="index">
                                        <td><Skeleton width="9rem" class="mb-2"></Skeleton></td>
                                ...
                                <tbody v-else>
                                    <tr v-for="(item, index) in data" :key="index">
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ displayStatus(item) }}</td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ item.price }}</td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ item.year }}</td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ displayMakeModel(item) }}</td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ item.mileage }}</td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ item.location }}</td>
