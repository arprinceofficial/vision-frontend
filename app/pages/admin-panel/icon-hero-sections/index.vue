...
const loadData = async () => {
	...
	permissions.value = {}
	try {
		const getData = await $fetchAdmin('v1/admin/icon-hero-sections/all', {
			method: 'POST',
			body: {
				paginate: true,
				page: 1,
		...
		response_modal.value = {
			status: false,
			message: e?.response?._data?.message || 'Failed to load icon hero section.',
		}
	} finally {
	...
	<div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
		<div class="h-full w-full overflow-auto">
			<div class="w-full flex flex-wrap justify-between items-center gap-4 mb-4">
				<div>
					<h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Icon Hero Section</h2>
				</div>
				<!-- <Skeleton v-if="isLoading" width="10rem" height="2.5rem" borderRadius="10px" />
				<Button
					v-else-if="permissions?.add || permissions?.edit"
					:label="hasHeroSection ? 'Update Icon Hero Section' : 'Create Icon Hero Section'"
			...
			<div class="pb-2 flex flex-col justify-between w-full">
				<div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
					<div class="border-b border-gray-200">
						<h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Current Icon Hero Section</h4>
					</div>
					...
					<div class="p-4">
						...
						>
							...
							<h5 class="text-base font-semibold text-gray-800 dark:text-gray-100">No Icon Hero Section Yet</h5>
							<p class="text-sm text-gray-500 mt-1 mb-4">Create the first icon hero section to activate this module.</p>
							<Button v-if="permissions?.add" label="Create Icon Hero Section" class="text-xs" @click="openManageModal" />
						</div>
					</div>

					<AddEdit :isOpenModal="isOpenModal" :item="item" :modalTitle="modalTitle" :data="data" @close="cancelModal"
