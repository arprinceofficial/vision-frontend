...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`v1/admin/icon-hero-sections/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin('v1/admin/icon-hero-sections', {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
...
<template>
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '40rem' }" @update:visible="$emit('close')">
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ isUpdateMode ? 'Update' : 'Create' }} Icon Hero Section</h4>
            </div>
        </template>
