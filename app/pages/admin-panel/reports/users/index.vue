...
const fetchData = async (page = 1) => {
    ...
    try {
        let url = `v1/admin/reports/users?page=${page}`;
        ...
        if (response?.data) {
            ...
            lastPage.value = response.data.users?.last_page || 1;
            paginationConfig.value.data = response.data.users || [];
        }
    } catch (error) {
...
const handleExport = async (format) => {
    let url = `v1/admin/reports/users?format=${format}`;
    if (search.value) url += `&search=${encodeURIComponent(search.value)}`;
    if (date.value) {
        const dates = Array.isArray(date.value) ? date.value : [date.value];
        const formattedDates = dates.map(d => d ? new Date(d).toLocaleDateString('en-CA') : '').filter(Boolean);
    ...
    try {
        ...
        link.download = `users_report_${new Date().toISOString().split('T')[0]}.${extension}`;
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-4">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Users Report</h1>
            </div>
        </div>
        ...
        <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-8">
            <div class="flex flex-wrap md:flex-nowrap items-center gap-4 w-full md:w-auto">
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <LazyInputText type="text" v-model="search" @keyup.enter="fetchData(1)" class="w-full md:w-auto" placeholder="Search user..." />
                </div>
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <table class="table-auto w-full dark:text-slate-300">
                        <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                            <tr>
                                ...
                                <th class="px-2 py-3 whitespace-nowrap"><div class="font-semibold text-left">Role</div></th>
                                <th class="px-2 py-3 whitespace-nowrap"><div class="font-semibold text-left">Status</div></th>
                                <th class="px-2 py-3 whitespace-nowrap"><div class="font-semibold text-left">Registered At</div></th>
                            </tr>
                        </thead>
                        <tbody class="text-sm divide-y divide-slate-200">
                            ...
                            <tr v-for="user in dataList" :key="user.id" v-else>
                                <td class="px-2 first:pl-5 py-3 whitespace-nowrap">
                                    <div class="text-left font-medium">{{ user.user_info?.first_name || user.first_name || 'N/A' }} {{ user.user_info?.last_name || user.last_name || '' }}</div>
                                ...
                                <td class="px-2 py-3 whitespace-nowrap">
                                    <div class="text-left capitalize">{{ user.role || 'N/A' }}</div>
                                </td>
                                <td class="px-2 py-3 whitespace-nowrap">
                                    <div class="text-left">
                                        <div class="inline-flex font-medium rounded-full text-center px-2.5 py-0.5" 
                                            :class="user.status == 1 ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'">
                                            {{ user.status == 1 ? 'Active' : 'Inactive' }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-2 py-3 whitespace-nowrap">
                                    <div class="text-left">{{ formatDate(user.created_at) }}</div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
