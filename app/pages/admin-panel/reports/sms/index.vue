...
const handleExport = async (format) => {
    ...
    try {
        ...
        link.download = `sms_logs_report_${new Date().toISOString().split('T')[0]}.${extension}`;
                ...
                <div class="overflow-x-auto" v-else>
                    <table class="table-auto w-full dark:text-slate-300">
                        <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                            <tr>
                                <th class="px-2 first:pl-5 py-3 whitespace-nowrap"><div class="font-semibold text-left">Phone Number</div></th>
                        ...
                        <tbody class="text-sm divide-y divide-slate-200">
                            ...
                            <tr v-for="log in dataList" :key="log.id" v-else>
                                <td class="px-2 first:pl-5 py-3 whitespace-nowrap">
                                    <div class="text-left font-medium">{{ log.number || 'N/A' }}</div>
                                    <div class="text-xs text-slate-500">ID: {{ log.id }}</div>
                                </td>
                                <td class="px-2 py-3">
                                    <div class="text-left truncate max-w-xs" :title="log.sms_body">{{ log.sms_body || 'N/A' }}</div>
                                </td>
