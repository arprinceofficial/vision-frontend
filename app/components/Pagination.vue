...
// Keep current_pg in sync with API data in ajax mode (e.g. inside modals where route doesn't change)
watch(() => data.value?.current_page, (newPage) => {
    if (newPage && action.value === 'ajax') {
        current_pg.value = parseInt(newPage)
        pg_list.value = {}
        first_page_display.value = ''
        last_page_display.value = ''
        init_pg_data()
    }
})
...
<template>
    <div class="flex items-center justify-between border-t border-gray-200 py-3">
        ...
        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-200 pr-2">
                    Showing
                    {{ ' ' }}
